...
export function serializeSettingsResponse(row: SettingsRow) {
  return {
    ...
    retentionPromptedAt: row.retentionPromptedAt,
...
export function fetchDownloadClients() {
  return db.select(clientColumns).from(downloadClients).orderBy(downloadClients.createdAt)
}

export type DownloadClientListRow = Awaited<ReturnType<typeof fetchDownloadClients>>[number]

export function serializeDownloadClientResponse(row: DownloadClientListRow) {
  return {
    ...row,
