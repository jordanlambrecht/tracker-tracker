...
export async function generateBackupPayload(): Promise<BackupPayload> {
  ...
  const rawSnapshots = await db
    ...
    .orderBy(trackerSnapshots.id)
  const snapshotsPayload = rawSnapshots.map((s) =>
    serializeRow(s as Record<string, unknown>)
  )
  ...
  const rawRoles = await db
    .select()
    .from(trackerRoles)
    .orderBy(trackerRoles.id)
  const rolesPayload = rawRoles.map((r) =>
  ...
  const rawTagGroups = await db
    .select()
    .from(tagGroups)
    .orderBy(tagGroups.id)
  const tagGroupsPayload = rawTagGroups.map((g) =>
    serializeRow(g as Record<string, unknown>)
  )

  // tagGroupMembers — include all columns
  const rawTagGroupMembers = await db
    .select()
    .from(tagGroupMembers)
    .orderBy(tagGroupMembers.id)
  ...
  const rawClientSnapshots = await db
    .select()
    .from(clientSnapshots)
    .orderBy(clientSnapshots.id)
...
export function validateBackupJson(
  payload: unknown
): asserts payload is BackupPayload {
  ...
  for (const [k, v] of Object.entries(
    ...
    if (typeof v !== "number" || !Number.isInteger(v) || v < 0) {
      throw new Error(
        `Backup validation: manifest.counts.${k} must be a non-negative integer`
      )
    }
  }
  ...
  if (!HEX_64_RE.test(settings.encryptionSalt)) {
    throw new Error(
      "Backup validation: settings.encryptionSalt must be exactly 64 hex characters"
    )
  }

  if (settings.backupScheduleFrequency !== undefined) {
    assertString(
      settings.backupScheduleFrequency,
      "settings.backupScheduleFrequency"
    )
  ...
  for (let i = 0; i < p.trackers.length; i++) {
    ...
    if (t.name.length === 0 || t.name.length > 100) {
      throw new Error(
        `Backup validation: ${prefix}.name must be between 1 and 100 characters`
      )
    }
    ...
    if (t.encryptedApiToken.length === 0) {
      throw new Error(
        `Backup validation: ${prefix}.encryptedApiToken must not be empty`
      )
    }
    ...
    if (t.color !== null && t.color !== undefined) {
      ...
      if (!HEX_COLOR_RE.test(t.color)) {
        throw new Error(
          `Backup validation: ${prefix}.color must be a valid hex color or null`
        )
      }
    }
  }
  ...
  for (let i = 0; i < p.trackerSnapshots.length; i++) {
    ...
    try {
      ...
      throw new Error(
        `Backup validation: ${prefix}.uploadedBytes must be parseable as BigInt`
      )
    }
    ...
    try {
      ...
      throw new Error(
        `Backup validation: ${prefix}.downloadedBytes must be parseable as BigInt`
      )
    }
  ...
  for (let i = 0; i < p.downloadClients.length; i++) {
    ...
    if (!Number.isInteger(c.port) || c.port < 1 || c.port > 65535) {
      throw new Error(
        `Backup validation: ${prefix}.port must be an integer between 1 and 65535`
      )
    }
    ...
    if (c.encryptedUsername.length === 0) {
      throw new Error(
        `Backup validation: ${prefix}.encryptedUsername must not be empty`
      )
    }
    ...
    if (c.encryptedPassword.length === 0) {
      throw new Error(
        `Backup validation: ${prefix}.encryptedPassword must not be empty`
      )
    }
  }
  ...
  for (const row of toDelete) {
    if (row.storagePath !== null) {
      ...
      if (!resolved.startsWith(base + path.sep)) {
        log.error(
          `Backup prune: path ${resolved} is outside base ${base}, skipping file deletion`
        )
      } else {
        try {
          ...
          log.error(
            err,
            `Backup prune: failed to delete file ${resolved}`
          )
        }
      }
    }
